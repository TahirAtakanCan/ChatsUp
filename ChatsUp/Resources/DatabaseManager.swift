    ...
    public func insertUser(with user: ChatsUpUser, completion: @escaping (Bool) -> Void) {
        database.child(user.safeEmail).setValue([
            ...
            strongSelf.database.child("users").observeSingleEvent(of: .value, with: { snapshot in
                ...
                strongSelf.database.child("users").setValue(newCollecttion, withCompletionBlock: { error, _ in
                    guard error == nil else {
                        completion(false)
                        return
                    }
...
/*        "id": String,
        ...
        database.child("\(conversation)/messages").observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            strongSelf.database.child("\(conversation)/messages").setValue(currentMessages) { error, _ in
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    strongSelf.database.child("\(conversation)/messages").setValue(databaseEntryConversations, withCompletionBlock: { error, _ in
                        guard error == nil else {
                            completion(false)
                            return
                        }
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                            ...
                            databaseEntryConversations = [[String: Any]]()
                            ...
                            if var otherUserConversations = snapshot.value as? [[String: Any]] {
                                ...
                                // current collection does not exitst
                                let newConversationData: [String: Any] = [
                                    "id": conversation,
                                    "other_user_email": DatabaseManager.safeEmail(emailAddress: currentEmail),
                                    "name": currentName,
    ...
    public func deleteConversaiton(conversationId: String, completion: @escaping (Bool) -> Void) {
        ...
        ref.observeSingleEvent(of: .value) { snapshot in
            if var conversation = snapshot.value as? [[String: Any]] {
                ...
                conversation.remove(at: positionToRemove)
                ref.setValue(conversation, withCompletionBlock: { error, _ in
                    guard error == nil else {
                        completion(false)
                        print("failed to write new conversation array")
                        return
                    }
                    print("deleted cconversation")
                    completion(true)
                })
            }
        }
